...
for key, value in binds.keyboardDefault["Flat"] {
	if Util.IsArray(value) && value.Length == 2 {
		if !binds.keyboardDefault["Moded"].Has(key)
			binds.keyboardDefault["Moded"][key] := Map()

		binds.keyboardDefault["Moded"][key].Set("+", [value[2], value[1]])
	}
}
...
defaultBindsOptions() {
	...
	if letterI_Option = "Separated" {
		binds.common["Moded"]["I"].Set("<+", ["lat_c_let_i", "lat_s_let_i_dotless"])
		binds.common["Flat"].Set("I", ["lat_c_let_i_dot_above", "lat_s_let_i"])
	} else if letterI_Option = "Hybrid" {
		binds.common["Moded"]["I"].Set("<+", ["lat_c_let_i_dot_above", "lat_s_let_i_dotless"])
	}
} defaultBindsOptions()


binds.important := BindList(binds.important["Flat"], binds.important["Moded"])
binds.keyboardDefault := BindList(binds.keyboardDefault["Flat"], binds.keyboardDefault["Moded"])
binds.supToSubDigits := BindList(binds.supToSubDigits["Flat"], binds.supToSubDigits["Moded"])
binds.subToSupDigits := BindList(binds.subToSupDigits["Flat"], binds.subToSupDigits["Moded"])
binds.common := BindList(binds.common["Flat"], binds.common["Moded"])
binds.diacritic := BindList(binds.diacritic["Flat"], binds.diacritic["Moded"])
binds.common.mapping := binds.common.mapping.MergeWith(binds.diacritic.mapping)