Class BindList {
	...
	static Get() {
		...
		if letterI_Option = "Separated" {
			bindingMaps.common["Moded"]["I"].Set("<+", ["lat_c_let_i", "lat_s_let_i_dotless"])
			bindingMaps.common["Flat"].Set("I", ["lat_c_let_i_dot_above", "lat_s_let_i"])
		} else if letterI_Option = "Hybrid" {
			bindingMaps.common["Moded"]["I"].Set("<+", ["lat_c_let_i_dot_above", "lat_s_let_i_dotless"])
		}

		for bindings, value in bindingMaps.OwnProps() {
			bindingMaps.%bindings% := BindList(bindingMaps.%bindings%["Flat"], bindingMaps.%bindings%["Moded"])
		}

		bindingMaps.common.mapping := bindingMaps.common.mapping.MergeWith(bindingMaps.diacritic.mapping)
