Class MyRecipes {
	...
	static Editor(sectionName := [], recipesLV?) {
		...
		Constructor() {
			this.editorTitle := App.Title("+status+version") " — " Locale.Read("gui.recipes.create_or_edit")
			data := {
				section: "",
				name: "",
				recipe: "",
			...
			sectionLabel := recipeCreator.AddText("vSectionLabel x" commonX() " y" commonY() " w150 BackgroundTrans", Locale.Read("gui.recipes.create_or_edit.section"))
			...
			nameLabel := recipeCreator.AddText("vNameLabel x" commonX() " y" commonY(55) " w150 BackgroundTrans", Locale.Read("gui.recipes.create_or_edit.name"))
			...
			getResultLabel() {
				return Locale.Read("dictionary.result") ": " Util.StrVarsInject(Locale.Read("gui.recipes.create_or_edit.overflow_properties"), totalLen, pages)
			}
