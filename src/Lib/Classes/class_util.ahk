Class Util {
	...
	static StrFormattedReduce(str, maxLength := 32, removeLineBreaks := False) {
		...
		local output := StrLen(str) > maxLength ? "[ " SubStr(str, 1, maxLength) " " Chr(0x2026) " ] ⟨ " this.StrVarsInject(Locale.Read("gui.recipes.create_or_edit.overflow_properties"), totalLen, pages) " ⟩" : str
		if removeLineBreaks {
