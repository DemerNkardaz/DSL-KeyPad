Class CharacterInserter {
	...
	static GetBlock(input := "0x0000", isFor := "Unicode") {
		if isFor = "Unicode" {
			for block, name in this.unicodeBlocks {
				split := StrSplit(block, "...")
				startBlockNum := Number(split[1])
				endBlockNum := Number(split[2])
